...
def _validate_password_strength(password: str, status: int = 400) -> None:
    ...
    if not re.search(r"[A-Z]", password):
        raise HTTPException(status_code=status, detail="Password must contain at least one uppercase letter")
...
def auth_delete_data(response: Response, request: Request) -> dict[str, Any]:
    ...
    from web_server import (
        _cookie_user,
        _profile_rw, _safe_profile, _save_profile, _purge_profile_user_data,
        store,
    )
    enforce_same_origin(request)
