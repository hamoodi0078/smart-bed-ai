...
def mobile_dashboard(request: Request, current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
    from web_server import mobile_dashboard as _ws_handler
    return _ws_handler(request=request)


@router.get("/v1/sleep/overview")
async def sleep_overview(request: Request, current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
    from web_server import (
        _sleep_readiness_score, _sleep_readiness_explanation,
        _recommended_scene_for_sleep_overview, _sleep_quick_actions,
    )
