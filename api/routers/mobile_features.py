...
async def register_push_token(request: Request, current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
...
def mobile_timeline(request: Request, current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
...
def first_3_nights(request: Request, current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
    from web_server import mobile_first_3_nights as _ws
    return _ws(request=request)


@router.post("/v1/mobile/first-3-nights/complete")
async def first_3_nights_complete(request: Request, current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
...
async def nightly_summary_feedback(request: Request, current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
...
def beta_metrics(request: Request, current_user: dict = Depends(get_current_user)) -> dict[str, Any]:
...
async def mobile_user_actions(request: Request, current_user: dict = Depends(get_current_user)) -> Any:
