...
class ReflectionState:
    def __init__(
        self, active: bool = False, step: Optional[str] = None, started_at_utc: Optional[str] = None
    ):
        self.active = active
        self.step = step
    ...
    def to_dict(self):
        return {"active": self.active, "step": self.step, "started_at_utc": self.started_at_utc}
...
REFLECTION_KEYWORDS = ["daily reflection", "daily summary", "how was my day", "end of the day"]
...
POSITIVE_MSG = "That's wonderful! Gratitude is powerful. Let's plan to repeat what worked. Wishing you restful sleep."
NEGATIVE_MSG = "It's okay to have tough days. Pick one small thing to improve tomorrow. Remember, every step counts. May you find ease and blessing."
...
def start_reflection_cmd(state: ReflectionState) -> CommandResult:
    ...
    return CommandResult(
        ...
        effects=(Effect(kind="update_reflection_state", payload=state.to_dict()),),
        followup_state={},
    )
