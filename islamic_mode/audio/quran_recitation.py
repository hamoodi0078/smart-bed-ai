...
class QuranRecitationService:
    ...
    def play_ayah(
        self, reciter_id: str, surah: int, ayah: int, download_if_missing: bool = True
    ) -> tuple[bool, str]:
        """
        Play a single ayah.
    ...
    def create_surah_playlist(
        self, reciter_id: str, surah: int, total_ayahs: int
    ) -> tuple[bool, str]:
        """
        ...
        self._current_playlist = []
        for ayah in range(1, total_ayahs + 1):
            self._current_playlist.append({"reciter": reciter_id, "surah": surah, "ayah": ayah})
    ...
    def play_next(self, download_if_missing: bool = True) -> tuple[bool, str]:
        ...
        return self.play_ayah(
            item["reciter"], item["surah"], item["ayah"], download_if_missing=download_if_missing
        )
    ...
    def clear_cache(self, reciter_id: Optional[str] = None) -> dict:
        ...
        return {"deleted": deleted, "failed": failed, "reciter": reciter_id or "all"}
