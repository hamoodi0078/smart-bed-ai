...
def send_whatsapp_notification(method: str, phone: str, user_id: str, extra: dict) -> bool:
    ok = _enqueue(
        "send_whatsapp_notification", method=method, phone=phone, user_id=user_id, extra=extra
    )
...
def send_weekly_report_notification(user_id: str, template_vars: dict | None = None) -> bool:
    ok = _enqueue(
        "send_weekly_report_notification", user_id=user_id, template_vars=template_vars or {}
    )
